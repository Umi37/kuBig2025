...
int main(void)
{
    ...
    {
        int temp_raw = (temp_msb << 8) | (temp_lsb & 0xFC); // 하위 2 비트 제거 (하위2비트가 다른기능을 함)
        // 0x11111100
        float temperature = -46.85 + 175.72*(temp_raw / 65535.0); // 65535 로 나눠야 섭씨로 출력
    ...
    if(temp_msb >= 0 && temp_lsb >= 0)
    {
        int rh_raw = (rh_msb << 8) | (rh_lsb & 0xFC); // 하위 2 비트 제거 (하위2비트가 다른기능을 함)
        // 0x11111100
        float relative_humidity = -6 + 125.0*(rh_raw / 65535.0); // 65535 로 나눠야 섭씨로 출력
        printf("습도: %.2f %% 도 - 습도 센서 정상 \n", relative_humidity);
    }
    else {
        printf("습도 읽기 실패 \n");
    }
