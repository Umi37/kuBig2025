...
int main()
{
    ...
    Mat dst1, dst2, dst3, dst4; // Destination images for arithmetic operations
    add(img, img2, dst1);
    addWeighted(img, 0.5, img2, 0.5, 0, dst2);
    subtract(img, img2, dst3);
    absdiff(img, img2, dst4); 

    images.push_back(dst1); // Add
    images.push_back(dst2); // AddWeighted
    images.push_back(dst3); // Subtract
    images.push_back(dst4); // AbsDiff
